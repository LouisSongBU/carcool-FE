...
export type PagedResp<T = any> = {
  ...
  // 获取“希望即将到期”列表（支持分页；兼容老数组返回）
  export function fetchPotentialExpirationList(
    displayName: string,
    days: number,
    page = 1,
    size = 100
    ...
    return api
      .get("/potential-expiration/expirations", {
...
export async function fetchPotentialExpirationAllByDays(days: number) {
  const resp = await fetch(`/api/potential-expiration/export-all?days=${days}`, {
    credentials: "include",
  });
