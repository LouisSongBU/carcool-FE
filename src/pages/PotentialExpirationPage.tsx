...
import styles from "./PotentialExpirationPage.module.css";
import { fetchPotentialExpirationList, updatePotentialExpirationDay, fetchPotentialExpirationAllByDays } from "../api/PotentialExpirationApi";
...
const PotentialExpirationPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchPotentialExpirationList(displayName, days, page, size)
  ...
  const onExportAll = async () => {
    try {
      ...
      const rows = await fetchPotentialExpirationAllByDays(days);
      if (!rows?.length) {
        alert("当前条件下没有可导出的数据～");
        return;
      }
      ...
      const filename = `希望客户到期_${yyyy}-${mm}-${dd}`;
  ...
  const onConfirmDays = async () => {
    if (inputDays < 0 || inputDays > 90) { setInputError("请输入 0 ~ 90 之间的数字"); return; }
    if (!window.confirm("提交后会更改提前天数设置，是否继续？")) return;
    try {
      setLoading(true);
      await updatePotentialExpirationDay(displayName, inputDays);
      userInfo.insuredExpirationDay = inputDays;
      sessionStorage.setItem("userInfo", JSON.stringify(userInfo));
      setDays(inputDays);
      setPage(1);
    } catch { setError("天数设置失败，请重试！"); }
  ...
  return (
    <div className={styles.expirationPage}>
      <div className={styles.topBar}>
        <span className={styles.title}>希望客户即将到期 <span className={styles.count}>{total}</span> 人</span>
        <div className={styles.daysSelect}>
          ...
          <span>天</span>
          <button className={styles.confirmBtn} onClick={onConfirmDays} disabled={!!inputError || inputDays === days}>确认</button>
...
export default PotentialExpirationPage;
