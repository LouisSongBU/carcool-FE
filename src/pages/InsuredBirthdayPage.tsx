...
import styles from "./InsuredBirthdayPage.module.css";
import { fetchInsuredBirthdayList, fetchInsuredBirthdayAll } from "../api/insuredBirthdayApi";
...
const InsuredBirthdayPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchInsuredBirthdayList(displayName, page, size)
  ...
  const onExportAll = async () => {
    try {
      ...
      const rows = await fetchInsuredBirthdayAll();
      if (!rows?.length) {
        alert("当前条件下没有可导出的数据～");
        return;
      }
      ...
      const filename = `车险客户生日_${yyyy}-${mm}-${dd}`;
  ...
  return (
    <div className={styles.expirationPage}>
      <div className={styles.topBar}>
        <span className={styles.title}>已保客户过生日 <span className={styles.count}>{total}</span> 人</span>
      </div>
...
export default InsuredBirthdayPage;
